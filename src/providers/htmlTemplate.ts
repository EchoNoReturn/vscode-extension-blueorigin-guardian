...
export const cveInfoTemplate = (data: CveInfo): string => {
  ...
  <div>${renderMetchFileList(data.proj_files, 'openFile')}</div>
  <h3>修复建议</h3>
  <div>请参考<a href="http://nvd.nist.gov/vuln/detail/${data.cve}">http://nvd.nist.gov/vuln/detail/${data.cve}</a>进行修复</div>`;
};
